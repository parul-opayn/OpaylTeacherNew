...
//    }
    ...
    func hitApi(requests : Request , completion : @escaping BaseAPICompletion ) {
        ...
        defer {
            ...
            print("Params",requests.parameters)
        }
        ...
        self.task?.responseJSON { [weak self] (response) in
            self?.serializedResponse(withResponse: response, clouser: { (receivedData, message, response) in
                if message?.lowercased().contains("unauthenticated") ?? false{
        ...
        //let name = getScreenName(name: Singleton.sharedInstance.controllerName)
        //print("This screen has been pushed \(name)")
    }
       ...
       // NetworkLogger.log(response: response.response ?? URLResponse())
        ...
        if response.response != nil {
            ...
            if NetworkReachabilityManager()!.isReachable{
                ...
                clouser(nil,"Unable to Connect to Internet", 2)
                print("No! internet isn't available.")
            }
        }
    }
}
