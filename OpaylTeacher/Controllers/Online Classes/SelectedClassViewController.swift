...
class SelectedClassViewController: UIViewController {
    ...
    var dataArray = ["Messages","Overview","Study Material","Reviews","More"]
    var selectedIndex = 1
    ...
    override func viewDidLoad() {
        ...
        bothSideIcons(titleString: className ?? "IELTS Preparation", titleType: .large, leftIcon: UIImage(), rightIcon: UIImage(named: "dots") ?? UIImage(), titleColor: .black, leftBtnType: .back, rightBtnType: .options)
        navigationWithBack(navtTitle: className ?? "IELTS Preparation", titleType: .large, titleColor: .black)
        ...
        dropDown.dataSource = ["Generate Live Class", "Add Custom Url"]
    }
...
extension SelectedClassViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        case 3:
            ...
            if viewModel.classDetail?.reviews?.count ?? 0 == 0{
            ...
            return  viewModel.classDetail?.reviews?.count ?? 0
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 2:
            ...
            cell.lockImage.isHidden = true
            cell.goForwardBtn.isHidden = true
        ...
        case 3:
            ...
            if  viewModel.classDetail?.reviews?.count ?? 0 > 0{
                ...
                let model = viewModel.classDetail?.reviews?[indexPath.row]
                cell.nameLbl.text = (model?.userID?.name ?? "").capitalized
                cell.ratingView.rating = Double(model?.rating ?? 0)
                cell.ratingLbl.text = "\(model?.rating ?? 0)"
                cell.reviewerImage.sd_setImage(with: URL(string: model?.profile?.image ?? ""), placeholderImage: UIImage(named: "placeholderImage"), options: .highPriority, context: nil)
                cell.reviewLbl.text = model?.content ?? ""
                let reviewDate = Singleton.sharedInstance.UTCToLocalDate(date: model?.createdAt ?? "", fromFormat: "yyyy-MM-dd HH:mm:ss", toFormat: "yyyy-MM-dd")
                cell.timeSpanLbl.text = Date().offset(from: reviewDate)
            }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 2:
          ...
          //  if !isMyClass{
           ...
           // }
           // else{
                let vc = storyBoardIdentifiers.main.getStoryBoard().instantiateViewController(withIdentifier: "PDFViewController") as! PDFViewController
...
extension SelectedClassViewController:DocumentPickerDelegate{
    ...
    func selectedDocxData(fileData: Data?, fileType: String?) {
        ...
        if let data = fileData, let type = fileType{
            ...
            let vc = storyBoardIdentifiers.main.getStoryBoard().instantiateViewController(withIdentifier: "ChatCaptionViewController") as! ChatCaptionViewController
            vc.fileData = data
            vc.mediaType = type
...
extension SelectedClassViewController{
    ...
    func reviewsListing(){
        ...
        viewModel.reviewsListing(course_id: "\(classId)", completion: {(isSuccess, msg) in
...
extension SelectedClassViewController:customNavigationDelegates{
